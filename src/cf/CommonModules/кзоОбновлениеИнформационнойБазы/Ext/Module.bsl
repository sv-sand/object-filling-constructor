...
Процедура ПриДобавленииПодсистемы(Описание) Экспорт
	...
	Описание.Имя    = "КонструкторЗаполненияОбъектов";
	Описание.Версия = "1.0.0.1";
	Описание.ИдентификаторИнтернетПоддержки = "OFC";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура           = "кзоОбновлениеИнформационнойБазы.ИзменитьСтруктуруХранилищаНастроек";
...
Процедура ИзменитьСтруктуруХранилищаНастроек() Экспорт
	
	Выборка = Справочники.кзоПравилаЗаполнения.Выбрать();
	Пока Выборка.Следующий() Цикл
		ИзменитьСтруктуруХранилищаНастроекСправочника(Выборка.Ссылка);		
	КонецЦикла;
	
	Выборка = Справочники.кзоПравилаПроверкиЗаполнения.Выбрать();
	Пока Выборка.Следующий() Цикл
		ИзменитьСтруктуруХранилищаНастроекСправочника(Выборка.Ссылка);		
	КонецЦикла;
...
Процедура ИзменитьСтруктуруХранилищаНастроекСправочника(Ссылка)
	...
	Настройки = Объект.ХранилищеНастроек.Получить(); 
	Если Настройки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	НовыеНастройки = Новый Структура;
	Если Настройки.Свойство("КомпоновщикОтбор") Тогда
		НовыеНастройки.Вставить("НастройкиОтбора", Настройки.КомпоновщикОтбор);
	КонецЕсли;
	Если Настройки.Свойство("КомпоновщикПроверки") Тогда
		НовыеНастройки.Вставить("НастройкиПроверки", Настройки.КомпоновщикПроверки);
	КонецЕсли;
	...
	Объект.ХранилищеНастроек = Новый ХранилищеЗначения(НовыеНастройки);
